...
public class JADEAppletRequestProto extends AppletRequestProto {
    ...
    Codec c;
    String action;
    private static final Ontology o = DFAppletOntology.getInstance();
    AID receiver;
    AID parent;
    DFGUI gui;
    ...
    JADEAppletRequestProto(DFAppletCommunicator communicator, AID receiver, String actionName, Object description, Object parentDF, SearchConstraints constraints) throws FIPAException {
        ...
        List<Action> content = new ArrayList<>();
        content.add(act);
     ...
     */
    public List<DFAgentDescription> getResult() throws FIPAException, NotYetReady {
        if (notYetReady)
        ...
        Iterator<Object> i = r.getItems().iterator(); //this is the set of DFAgentDescription
        List<DFAgentDescription> l = new ArrayList<>();
        while (i.hasNext())
            l.add((DFAgentDescription) i.next());
        return l;
    ...
    protected void handleInform(ACLMessage msg) {
        try {
            ...
            if (this.action.equalsIgnoreCase(DFAppletVocabulary.FEDERATE)) {
                ...
                try {
                    List<DFAgentDescription> result = getResult();
                    dfApplet.setDescription(result.get(0));
                ...
                gui.refreshLastSearchResults((ArrayList<DFAgentDescription>) getResult(), parent);
                gui.showStatusMsg("Request processed. Ready for new request.");
