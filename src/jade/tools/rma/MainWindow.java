...
class MainWindow extends JFrame {
    ...
    private final Map<String, List<String>> addresses = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
    ...
    public MainWindow(rma anRMA) {
        ...
        tree.getTreeAgent().setNewPopupMenu(AgentTree.AGENT_TYPE, popA);
        tree.getTreeAgent().setNewPopupMenu(AgentTree.FROZEN_AGENT_TYPE, popFA);
        tree.getTreeAgent().setNewPopupMenu(AgentTree.CONTAINER_TYPE, popC);
        tree.getTreeAgent().setNewPopupMenu(AgentTree.REMOTE_PLATFORM_TYPE, popRP);
        JPopupMenu popupRemote = new JPopupMenu();
        JMenuItem temp = popupRemote.add(ActionProcessor.actions.get(ActionProcessor.CUSTOM_ACTION));
        temp.setIcon(null);
        temp = popupRemote.add(ActionProcessor.actions.get(ActionProcessor.REGISTERREMOTEAGENTWITHAMS_ACTION));
        temp.setIcon(null);
        temp.setEnabled(false);
        tree.getTreeAgent().setNewPopupMenu(AgentTree.REMOTE_AGENT_TYPE, popupRemote);
        tree.getTreeAgent().setNewPopupMenu(AgentTree.TREE_ROOT_TYPE, popP);
        JPopupMenu popLocalPlatform = new JPopupMenu();
        JMenuItem tmp = popLocalPlatform.add(ActionProcessor.actions.get(ActionProcessor.VIEWPLATFORM_ACTION));
        tmp.setIcon(null);
        popLocalPlatform.add(ActionProcessor.actions.get(ActionProcessor.MANAGE_MTPS_ACTION));
        tree.getTreeAgent().setNewPopupMenu(AgentTree.LOCAL_PLATFORM_TYPE, popLocalPlatform);
    ...
    public void disposeAsync() {
        ...
        SwingUtilities.invokeLater(toDispose::dispose);

    }

    public AgentTreeModel getModel() {
        return tree.getTreeAgent().getModel();
    }

    public void resetTree() {
        Runnable resetIt = () -> tree.getTreeAgent().clearLocalPlatform();
    ...
    public void addContainer(final String name, final InetAddress addr) {
        Runnable addIt = () -> {
            ...
            tree.getTreeAgent().addContainerNode(name, addr);
    ...
    public void removeContainer(final String name) {
        ...
        Runnable removeIt = () -> {
            tree.getTreeAgent().removeContainerNode(name);
    ...
    public void addAgent(final String containerName, final AID agentID, final String state, final String ownership) {
        ...
        Runnable addIt = () -> {
            ...
            StringBuilder agentAddresses = new StringBuilder();
            while (add.hasNext()) {
                agentAddresses.append(add.next()).append(" ");
            }
            tree.getTreeAgent().addAgentNode(agentName, agentAddresses.toString(), containerName, state, ownership);
        };
        SwingUtilities.invokeLater(addIt);
    }
    ...
    public void removeAgent(final String containerName, final AID agentID) {
        ...
        Runnable removeIt = () -> {
            String agentName = agentID.getName();
            tree.getTreeAgent().removeAgentNode(containerName, agentName);
        };
        SwingUtilities.invokeLater(removeIt);
    }
    ...
    public void modifyAgent(final String containerName, final AID agentID, final String state, final String ownership) {
        ...
        Runnable modifyIt = () -> {
            String agentName = agentID.getName();
            tree.getTreeAgent().modifyAgentNode(containerName, agentName, null, state, ownership);
        };
    ...
    public void moveAgent(final String fromContainer, final String toContainer, final AID agentID) {
        ...
        Runnable moveIt = () -> {
            String agentName = agentID.getName();
            tree.getTreeAgent().moveAgentNode(fromContainer, toContainer, agentName);
        };
    ...
    public void modifyFrozenAgent(final String oldContainer, final String newContainer, final AID agentID) {
        ...
        Runnable freezeIt = () -> {
            String agentName = agentID.getName();
            tree.getTreeAgent().freezeAgentNode(oldContainer, newContainer, agentName);
        };
    ...
    public void modifyThawedAgent(final String oldContainer, final String newContainer, final AID agentID) {
        ...
        Runnable thawIt = () -> {
            String agentName = agentID.getName();
            tree.getTreeAgent().thawAgentNode(oldContainer, newContainer, agentName);
        };
    ...
    public void addRemotePlatformFolder() {
        Runnable addIt = () -> {
            ...
            tree.getTreeAgent().setNewPopupMenu(AgentTree.REMOTE_PLATFORMS_FOLDER_TYPE, menu);
            tree.getTreeAgent().addRemotePlatformsFolderNode();
    ...
    public void addRemotePlatform(AID name, APDescription profile) {
        ...
        final AID ams = name;
        Runnable addIt = () -> tree.getTreeAgent().addRemotePlatformNode(ams, desc);
        SwingUtilities.invokeLater(addIt);
    }


    public void refreshRemoteAgentsInRemotePlatform(final APDescription platform, final Iterator<Object> remoteAgents) {
        // Add an agent to a specified AMS
        Runnable addIt = () -> {
            // Clear all previous agents
            tree.getTreeAgent().clearRemotePlatformAgents(platform.getName());

            // Add new current agents
            while (remoteAgents.hasNext()) {
                AMSAgentDescription agent = (AMSAgentDescription) remoteAgents.next();
                tree.getTreeAgent().addRemoteAgentNode(agent, platform.getName());
            }
        };
        SwingUtilities.invokeLater(addIt);
    }
    ...
    public void removeRemotePlatform(final String platformName) {
        ...
        Runnable addIt = () -> tree.getTreeAgent().removeRemotePlatformNode(platformName);
        SwingUtilities.invokeLater(addIt);
    }
    ...
    public void refreshLocalPlatformName(final String name) {
        ...
        Runnable refreshName = () -> tree.getTreeAgent().refreshLocalPlatformName(name);
    ...
    public boolean showInstallMTPDialog(jade.domain.JADEAgentManagement.InstallMTP imtp) {
        String[] names = containerNames.toArray(new String[0]);
