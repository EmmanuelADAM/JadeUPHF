...
public class Introspector extends ToolAgent {
    ...
    private final Set<AID> slowAgents = new HashSet<>();
    ...
    private final Map<AID, String> pendingReplies = new HashMap<>();
    ...
    public void toolSetup() {
        ...
        if (arguments != null) {
            for (Object argument : arguments) {
                parsePreloadDescription((String) argument);
            }
        }
    ...
    public void removeAgent(final AID name) {
        if (windowMap.containsKey(name)) {
            try {
                final MainWindow m = windowMap.get(name);
                myGUI.closeInternal(m);
    ...
    public void doDelete() {
        ...
        if (windowMap.containsKey(amsId)) {
            try {
                final MainWindow m = windowMap.get(amsId);
                myGUI.closeInternal(m);
    ...
    protected String preloadContains(String agentName) {
        for (Enumeration<String> enumeration = preload.keys(); enumeration.hasMoreElements(); ) {
    ...
    private class RequestListenerBehaviour extends SimpleAchieveREResponder {
        ...
        protected ACLMessage prepareResultNotification(ACLMessage request, ACLMessage response) {
            if (aa instanceof DebugOn requestDebugOn) {
                ...
                ArrayList<AID> agentsToDebug = requestDebugOn.getCloneOfDebuggedAgents();
                for (AID value : agentsToDebug) {
                    if (allAgents.contains(value)) {
                        addAgent(value);
                    } else {
                        ...
                        preload.put(value.getName(), filter);
                    }
                }
            } else {
                // DEBUG OFF
                DebugOff requestDebugOff = (DebugOff) aa;
                List<AID> agentsToDebug = requestDebugOff.getCloneOfDebuggedAgents();
                for (AID aid : agentsToDebug) {
                    removeAgent(aid);
                }
            }
