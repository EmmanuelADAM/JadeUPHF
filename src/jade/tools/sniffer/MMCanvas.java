   ...
   #DOTNET_INCLUDE_END*/
        try {
            if (typeCanv) {
                ...
                Iterator<Agent> it = al.getAgents();
                while (it.hasNext()) {
     ...
     #DOTNET_INCLUDE_END*/
            ...
            if ((!typeCanv)) {
                ...
                Iterator<Message> it = ml.getMessages();
                int AllReceiver = 0;
                while (it.hasNext()) {
		   ...
		   #DOTNET_INCLUDE_END*/
                    ...
                    if (conversationKey != null) {
                        if (mapToColor.containsKey(conversationKey)) {
                            colorIndex = mapToColor.get(conversationKey);
                        } else {
                            colorIndex = getNewColorIndex();
                            mapToColor.put(conversationKey, colorIndex);
                        }
                        messageColor = colorTable[colorIndex % colorTable.length];
                    } else {
		 ...
		 #DOTNET_INCLUDE_END*/
                    ...
                    int counter = 0;
                    for (Iterator<Message> i = ml.getMessages(); i.hasNext(); ) {
                        Message msg = i.next();
	   ...
	   #DOTNET_INCLUDE_END*/
                ...
                for (int t = 0; t <= AllReceiver; t++) {
                    ...
                    msgNumWrapped = t;
                    //#DOTNET_EXCLUDE_BEGIN
                    g.drawString(Integer.toString(msgNumWrapped), 10, timeUnitWidth * (t) + 15);
                    //#DOTNET_EXCLUDE_END
	      /*#DOTNET_INCLUDE_BEGIN
		  Font f = new Font("Arial", 10);
    ...
    private boolean checkNoSniffedVector(Agent agent) {
        ...
        if (noSniffAgents.size() == 0) return false;
            ...
            for (int i = 0; i < noSniffAgents.size(); i++) {
                agentToCompare = noSniffAgents.get(i);
