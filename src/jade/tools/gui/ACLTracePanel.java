...
//  ***EOF***
    ...
    public void addMessageNode(String direction, String theTime, ACLMessage theACL) {
        ...
        String aclString = theACL.toString().replace('\n', ' ');
        ...
        if (theACL.getAllReceiver().hasNext()) {
            theReceiver = theACL.getAllReceiver().next().getLocalName();
        }

        if (direction.startsWith("i")) {
            this.inMsgCount++;
            aclString = ACLMessage.getPerformative(theACL.getPerformative()) +
        ...
        if (contentLanguage.contains("SL")) {
            //Only format when SL
            try {
                plainContent = new SLFormatter().format(plainContent);
            } catch (Exception ex) {
                //too bad!
            }
        }
        ...
        int i = 0;
        aclModel.insertNodeInto(new DefaultMutableTreeNode(":act " + ACLMessage.getPerformative(theACL.getPerformative())), messageNode, i++);

        AID sender = theACL.getSender();
        if (sender != null) {
            aclModel.insertNodeInto(new DefaultMutableTreeNode(":sender " + sender), messageNode, i++);
        }
        ...
        if (replyBy != null) {
            aclModel.insertNodeInto(new DefaultMutableTreeNode(":reply-by " + replyBy), messageNode, i++);
        }
...
//  ***EOF***
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                ...
                while (line != null) {
                    ...
                    StringBuilder theMessageLine = new StringBuilder();
                    line = br.readLine();
                    while ((line != null) && (!line.equals(""))) {
                        theMessageLine.append("\n").append(line);
                        line = br.readLine();
                    }
                    char[] chararray = theMessageLine.toString().toCharArray();
    ...
    void aclTree_mouseClicked(MouseEvent e) {
        try {
            ...
            if ((e.getModifiersEx() == InputEvent.BUTTON1_DOWN_MASK) && (e.getClickCount() == 2)) {
                if (currentACL == null) {
                    JOptionPane.showMessageDialog(null, "No ACL", "Select a Message", JOptionPane.ERROR_MESSAGE);
                    return;
                }
    ...
    private void jbInit() throws Exception {
        ...
        systemMenuItem.setFont(new Font("Dialog", Font.PLAIN, 11));
        ...
        zoomMenuItem.setFont(new Font("Dialog", Font.PLAIN, 11));
        ...
        stasticsMenuItem.setFont(new Font("Dialog", Font.PLAIN, 11));
        ...
        deleteMenuItem.setFont(new Font("Dialog", Font.PLAIN, 11));
        ...
        ascRadioButton.setFont(new Font("Dialog", Font.PLAIN, 12));
        ...
        descRadioButton.setFont(new Font("Dialog", Font.PLAIN, 12));
        ...
        sortComboBox.setFont(new Font("Dialog", Font.PLAIN, 12));
        ...
        saveQMenuItem.setFont(new Font("Dialog", Font.PLAIN, 11));
        saveQMenuItem.setMnemonic('A');
        saveQMenuItem.setText("Save ACLMessage Trace");
        saveQMenuItem.addActionListener(this::saveQMenuItem_actionPerformed);
        openQMenuItem.setBackground(Color.white);
        openQMenuItem.setFont(new Font("Dialog", Font.PLAIN, 11));
        ...
        clearQMenuItem.setFont(new Font("Dialog", Font.PLAIN, 11));
    ...
    private class ACLMessageNodeComparator implements Comparator<Object> {
        ...
        public int compare(Object o1, Object o2) {
            ...
            switch (mode) {
                ...
                case RECEIVER -> {
                    comp1 = (node1.getMessage().getAllReceiver().hasNext() ? node1.getMessage().getAllReceiver().next().getName() : "");
                    comp2 = (node2.getMessage().getAllReceiver().hasNext() ? node2.getMessage().getAllReceiver().next().getName() : "");
                }
                case PERFORMATIVE -> {
    ...
    private class AboutFrame extends JWindow {
        ...
        private void jbInit() throws Exception {
            ...
            logoLabel3.setFont(new Font("SansSerif", Font.BOLD, 60));
            ...
            jLabel4.setFont(new Font("Dialog", Font.BOLD, 20));
            ...
            switch (depth) {
                ...
                case 2:
                    ...
                    if (sValue.startsWith(":act ")) {
                        ...
                        String performative = sValue.substring(5);
