...
class SL2Ontology extends SL1Ontology implements SL2Vocabulary {
    ...
    protected SL2Ontology(String name, Ontology base, Introspector intro) {
        ...
        try {
            ...
            ps.add(IMPLIES_LEFT, PredicateSchema.getBaseSchema());
            ps.add(IMPLIES_RIGHT, PredicateSchema.getBaseSchema());
            ps = (PredicateSchema) getSchema(EQUIV);
            ps.add(EQUIV_LEFT, PredicateSchema.getBaseSchema());
            ps.add(EQUIV_RIGHT, PredicateSchema.getBaseSchema());
            ...
            ps.add(EXISTS_WHAT, VariableSchema.getBaseSchema());
            ps.add(EXISTS_CONDITION, PredicateSchema.getBaseSchema());

            ps = (PredicateSchema) getSchema(FORALL);
            ps.add(FORALL_WHAT, VariableSchema.getBaseSchema());
            ps.add(FORALL_CONDITION, PredicateSchema.getBaseSchema());

            ps = (PredicateSchema) getSchema(BELIEF);
            ps.add(BELIEF_AGENT, getSchema(AID));
            ps.add(BELIEF_CONDITION, PredicateSchema.getBaseSchema());

            ps = (PredicateSchema) getSchema(UNCERTAINTY);
            ps.add(UNCERTAINTY_AGENT, getSchema(AID));
            ps.add(UNCERTAINTY_CONDITION, PredicateSchema.getBaseSchema());

            ps = (PredicateSchema) getSchema(PERSISTENT_GOAL);
            ps.add(PERSISTENT_GOAL_AGENT, getSchema(AID));
            ps.add(PERSISTENT_GOAL_CONDITION, PredicateSchema.getBaseSchema());

            ps = (PredicateSchema) getSchema(INTENTION);
            ps.add(INTENTION_AGENT, getSchema(AID));
            ps.add(INTENTION_CONDITION, PredicateSchema.getBaseSchema());

            ps = (PredicateSchema) getSchema(FEASIBLE);
            ps.add(FEASIBLE_ACTION, VariableSchema.getBaseSchema());
            ps.add(FEASIBLE_CONDITION, PredicateSchema.getBaseSchema(), ObjectSchema.OPTIONAL);
