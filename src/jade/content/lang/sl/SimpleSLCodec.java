...
public class SimpleSLCodec extends StringCodec {
    ...
    private void encodeSlotsByName(AbsObject val, String[] slotNames, Ontology onto, StringBuffer str) throws CodecException {
        for (String slotName : slotNames) {
            AbsObject s = val.getAbsObject(slotName);
            if (s != null) {
                str.append(" :");
                str.append(slotName);
                str.append(" ");
    ...
    private void stringifyPrimitive(AbsPrimitive val, StringBuffer str) throws CodecException {
        ...
        switch (type) {
            ...
            case BasicOntology.DATE -> str.append(ISO8601.toString(val.getDate()));
            case BasicOntology.BYTE_SEQUENCE -> throw new CodecException("SL_does_not_allow_encoding_sequencesOfBytes");
            default -> str.append(val.getObject().toString());
        }
    }
    ...
    private AbsObject parseSimple(SimpleSLTokenizer p) throws CodecException {
        ...
        try {
            ...
            return AbsPrimitive.wrap(Double.parseDouble(val));
        } catch (Exception e) {
        }
        //#MIDP_EXCLUDE_END
        // Date
        ...
        // Variable
        if (val.startsWith("?")) {
            return new AbsVariable(val.substring(1), null);
        }
        // String
        if (val.startsWith("\"")) {
