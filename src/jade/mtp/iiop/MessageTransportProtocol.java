    ...
    public void deliver(String addr, Envelope env, byte[] payload) throws MTPException {
        try {
            ...
            if (objRef._non_existent())
                throw new MTPException("Bad IIOP server object reference:" + objRef);

            // Fill in the 'to' field of the IDL envelope
            Iterator<AID> itTo = env.getAllTo();
            List<AgentID> to = new ArrayList<>();
            while (itTo.hasNext()) {
                AID id = itTo.next();
            ...
            List<AgentID> intendedReceiver = new ArrayList<>();
...
class IIOPAddress implements TransportAddress {
    ...
    private void parseIOR(String s, String typeName) throws MTPException {
        try {
            ...
            switch (endianness) {
                case BIG_ENDIAN -> codecStrategy = new BigEndianCodec(hexString);
                case LITTLE_ENDIAN -> codecStrategy = new LittleEndianCodec(hexString);
                default -> throw new MTPException("Invalid endianness specifier");
            }
            ...
            for (int i = 0; i < seqLen; i++) {
                ...
                if (tag == TAG_INTERNET_IOP) {
                    ...
                    CDRCodec profileBodyCodec = switch (profile[0]) {
                        case BIG_ENDIAN -> new BigEndianCodec(profile);
                        case LITTLE_ENDIAN -> new LittleEndianCodec(profile);
                        default -> throw new MTPException("Invalid endianness specifier");
                    };
    ...
    private void buildIOR(String s, String typeName, short endianness) throws MTPException {
        ...
        if (poundPos == -1) {
            objectKey = s.substring(slashPos + 1);
            anchor = "";
        } else {
            objectKey = s.substring(slashPos + 1, poundPos);
            anchor = s.substring(poundPos + 1);
        }

        switch (endianness) {
            case BIG_ENDIAN -> codecStrategy = new BigEndianCodec(new byte[0]);
            case LITTLE_ENDIAN -> codecStrategy = new LittleEndianCodec(new byte[0]);
            default -> throw new MTPException("Invalid endianness specifier");
        }
        ...
        CDRCodec profileBodyCodec = switch (endianness) {
            case BIG_ENDIAN -> new BigEndianCodec(new byte[0]);
            case LITTLE_ENDIAN -> new LittleEndianCodec(new byte[0]);
            default -> throw new MTPException("Invalid endianness specifier");
        };
        ...
        byte[] objKey = objectKey.getBytes(StandardCharsets.US_ASCII);
        ...
        for (int i = 0; i < objKey.length; i++) {
            ...
            if (b != ASCII_PERCENT)
                ...
                String hexPair = new String(objKey, i + 1, 2, StandardCharsets.US_ASCII);
    ...
    private boolean isUnreservedURIChar(byte b) {
        ...
        return (b == ASCII_MINUS) || (b == ASCII_UNDERSCORE) || (b == ASCII_DOT) || (b == ASCII_BANG) || (b == ASCII_TILDE) ||
    ...
    private static class BigEndianCodec extends CDRCodec {
        ...
        public long readLongLong() {
            setReadAlignment(8);
            long result = ((long) readBuffer[readIndex++] << 56) + ((long) readBuffer[readIndex++] << 48);
            result += ((long) readBuffer[readIndex++] << 40) + ((long) readBuffer[readIndex++] << 32);
    ...
    private static class LittleEndianCodec extends CDRCodec {
        ...
        public int readLong() {
            setReadAlignment(4);
            return readBuffer[readIndex++] + (readBuffer[readIndex++] << 8) + (readBuffer[readIndex++] << 16) + (readBuffer[readIndex++] << 24);
        }
        ...
        public long readLongLong() {
            ...
            result += ((long) readBuffer[readIndex++] << 32) + ((long) readBuffer[readIndex++] << 40);
            result += ((long) readBuffer[readIndex++] << 48) + ((long) readBuffer[readIndex++] << 56);
            return result;
        }
