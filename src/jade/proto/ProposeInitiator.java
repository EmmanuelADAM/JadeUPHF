...
public class ProposeInitiator extends FSMBehaviour {
    ...
    public ProposeInitiator(Agent a, ACLMessage initiation, HashMap<String, List<ACLMessage>> mapMessagesList) {
        ...
        b = new OneShotBehaviour(myAgent) {
            ...
            public void action() {
                ...
                var allInitiations = prepareInitiations(ds.get(INITIATION_K));
                getMapMessagesList().put(ALL_INITIATIONS_K, allInitiations);
            }
        };
        ...
        // RECEIVE_REPLY
        replyReceiver = new MsgReceiver(myAgent, null, MsgReceiver.INFINITE, getMapMessagesList(), REPLY_KEY);
        ...
        b = new OneShotBehaviour(myAgent) {
            public void action() {
                handleAcceptProposal(getMapMessages().get(REPLY_KEY));
            }
        };
        b.setMapMessagesList(getMapMessagesList());
        ...
        b = new OneShotBehaviour(myAgent) {
            public void action() {
                handleRejectProposal(getMapMessages().get(REPLY_KEY));
            }
        };
        b.setMapMessagesList(getMapMessagesList());
        ...
        b = new OneShotBehaviour(myAgent) {
            ...
            public void action() {
                handleNotUnderstood(getMapMessages().get(REPLY_KEY));
            }
        };
        b.setMapMessagesList(getMapMessagesList());
        ...
        b = new OneShotBehaviour(myAgent) {
            ...
            public void action() {
                handleOutOfSequence(getMapMessages().get(REPLY_KEY));
            }
        };
        b.setMapMessagesList(getMapMessagesList());
    ...
    protected void initializeHashMap(ACLMessage msg) {
        initiation = msg;
        getMapMessages().put(INITIATION_K, initiation);
    ...
    protected void sendInitiations(List<ACLMessage> initiations) {
        ...
        for (ACLMessage proposal : initiations) {
            if (proposal != null) {
                ...
                for (Iterator<AID> receivers = proposal.getAllReceiver(); receivers.hasNext(); ) {
                    ...
                    String sessionKey = "R" + hashCode() + "_" + cnt;
                    ...
                    getMapMessages().put(r.toString(), toSend);
                    cnt++;
                }
    ...
    protected boolean checkInSequence(ACLMessage reply) {
        ...
        Session s = sessions.get(inReplyTo);
        if (s != null) {
    ...
    protected int checkSessions(ACLMessage reply) {
        ...
        if (getLastExitValue() == MsgReceiver.TIMEOUT_EXPIRED) {
            if (!allResponsesReceived) {
                ...
                List<String> sessionsToRemove = new ArrayList<>(sessions.size());
                sessions.forEach((k, v) -> {
                    if (v.getState() == Session.INIT) sessionsToRemove.add(k);
                });
                sessionsToRemove.forEach(s -> sessions.remove(s));
            } else {
        ...
        if (!allResponsesReceived) {
            ...
            allResponsesReceived = sessions.values().stream().noneMatch(s -> s.getState() == Session.INIT);
            if (allResponsesReceived) {
    ...
    public void reset(ACLMessage msg) {
        super.reset();
        replyReceiver.reset(null, MsgReceiver.INFINITE, getMapMessagesList(), getMapMessages(), REPLY_KEY);
        initiation = msg;
        sessions.clear();
