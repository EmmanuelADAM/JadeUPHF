...
public class SSIteratedAchieveREResponder extends SSResponder {
    ...
    public SSIteratedAchieveREResponder(Agent a, ACLMessage request) {
        this(a, request, new HashMap<>(), new HashMap<>());
    }
     ...
     * @deprecated
     */
    public SSIteratedAchieveREResponder(Agent a, ACLMessage request, HashMap<String, List<ACLMessage>> mapMessagesList, HashMap<String, ACLMessage> mapMessages) {
        // 5th parameter is false since in this protocol we treat the initiation message exactly as all subsequent incoming messages
        super(a, request, mapMessagesList, mapMessages, false);
    ...
    public void sendAgree(ACLMessage agree) {
        ...
        ReplySender.adjustReply(myAgent, agree, getMapMessages().get(REQUEST_KEY));
        myAgent.send(agree);
    }
    ...
    private static class RequestHandler extends OneShotBehaviour {
        ...
        public void action() {
            ...
            ACLMessage reply;
            try {
                reply = parent.handleRequest(getMapMessages().get(parent.REQUEST_KEY));
    ...
    private static class CancelHandler extends OneShotBehaviour {
        ...
        public void action() {
            SSIteratedAchieveREResponder parent = (SSIteratedAchieveREResponder) getParent();
            parent.handleCancel(getMapMessages().get(parent.CANCEL_KEY));
        }
