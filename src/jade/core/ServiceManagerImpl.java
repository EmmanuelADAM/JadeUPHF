...
public class ServiceManagerImpl implements ServiceManager, ServiceFinder {
    ...
    public synchronized void removeAddress(String addr) {
        ...
        try {
            ...
            if (CaseInsensitiveString.equalsIgnoreCase(ta1.getProto(), ta2.getProto())) {
                if (CaseInsensitiveString.equalsIgnoreCase(ta1.getPort(), ta2.getPort())) {
                    return Profile.compareHostNames(ta1.getHost(), ta2.getHost());
                }
            }
            return false;
        } catch (Exception e) {
    ...
    public void addNode(NodeDescriptor desc, ServiceDescriptor[] services) throws IMTPException, ServiceException, JADESecurityException {
        ...
        try {
            ...
            Vector<ServiceDescriptor> ss = new Vector<>(services != null ? services.length : 0);
            if (services != null) {
        ...
        } catch (IMTPException | ServiceException | JADESecurityException se) {
            throw se;
        } catch (Throwable t) {
    ...
    public void deactivateService(String name) throws IMTPException, ServiceException {
        ServiceDescriptor desc = localServices.get(name);
        if (desc != null) {
    ...
    public Service findService(String key) throws IMTPException, ServiceException {
        Service svc = null;
        ServiceDescriptor svcDsc = localServices.get(key);
        if (svcDsc != null) {
    ...
    private void uninstallServiceLocally(String name) throws IMTPException, ServiceException {
        ServiceDescriptor svcDsc = localServices.get(name);
        if (svcDsc != null) {
            Service svc = svcDsc.getService();
    ...
    private synchronized boolean reconnect() {
        if (status == ACTIVE_STATUS) {
            ...
            try {
                ...
                for (String o : backupManagers.keySet()) {
                    try {
                        myPlatformManager = backupManagers.get(o);
    ...
    private void handlePMRefreshed(String pmAddr) {
        ...
        for (Object service : services) {
            ServiceDescriptor svcDsc = (ServiceDescriptor) service;
            Service svc = svcDsc.getService();
    ...
    public Vector<ServiceDescriptor> getLocalServices() {
        Object[] services = localServices.values().toArray();
        Vector<ServiceDescriptor> ss = new Vector<>(services.length);
        for (Object service : services) {
            if (service instanceof ServiceDescriptor) {
                ss.addElement((ServiceDescriptor) service);
            }
        }
        return ss;
    }
