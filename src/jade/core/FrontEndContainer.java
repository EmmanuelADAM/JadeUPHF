...
class FrontEndContainer implements FrontEnd, AgentToolkit, Runnable {
    ...
    private Vector<Serializable> pending;
    ...
    void start(Properties p) {
        ...
        try {
            Vector<Object> specs = Specifier.parseSpecifierList(agents);
            Vector<Specifier> successfulAgents = new Vector<>();
            for (Enumeration<Object> en = specs.elements(); en.hasMoreElements(); ) {
        ...
        Vector<Object> svcClasses = Specifier.parseList(feServices, ';');
        for (Enumeration<Object> en = svcClasses.elements(); en.hasMoreElements(); ) {
        ...
        try {
            Vector<Object> specs = Specifier.parseSpecifierList(agents);
            // Start all agents
            Enumeration<Object> e = specs.elements();
    ...
    private void notifyListeners(JADEEvent ev) {
        synchronized (feListeners) {
            for (FEListener feListener : feListeners) {
                feListener.handleEvent(ev);
            }
        }
    }
    //#MIDP_EXCLUDE_END
     ...
     * @param receiver The name of the receiver agent.
     */
    ...
    public final void exit(boolean self) throws IMTPException {
        if (!exiting) {
            ...
            synchronized (localAgents) {
                Enumeration<Agent> e = localAgents.elements();
                while (e.hasMoreElements()) {
    ...
    public final void synch() throws IMTPException {
        synchronized (localAgents) {
            Enumeration<String> e = localAgents.keys();
            while (e.hasMoreElements()) {
    ...
    private Agent initAgentInstance(String name, String className, Object[] args) throws Exception {
        Agent agent;
        //#J2ME_EXCLUDE_BEGIN
    ...
    private void post(ACLMessage msg, String sender) {
        ...
        synchronized (pending) {
            ...
            pending.addElement((Serializable) msg.clone());
