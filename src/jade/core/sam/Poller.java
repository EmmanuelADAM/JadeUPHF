...
class Poller extends Thread {
    ...
    public synchronized void addHandler(SAMInfoHandler h, boolean first) {
        ...
        System.arraycopy(handlers, 0, newHandlers, k, handlers.length);
        if (!first) {
    ...
    public synchronized void removeHandler(SAMInfoHandler handler) {
        ...
        if (found) {
            ...
            // Copy current handlers
            int k = 0;
            for (SAMInfoHandler samInfoHandler : handlers) {
                if (samInfoHandler != handler) {
                    newHandlers[k++] = samInfoHandler;
                }
            }
            handlers = newHandlers;
        }
    ...
    void poll() {
        ...
        try {
            ...
            for (Service.Slice slice : slices) {
                SAMSlice s = (SAMSlice) slice;
