...
public class MainReplicationService extends BaseService {
    ...
    public Class<MainReplicationSlice> getHorizontalInterface() {
        try {
            return (Class<MainReplicationSlice>) Class.forName(MainReplicationSlice.NAME + "Slice");
    ...
    public void boot(Profile p) throws ServiceException {
        try {
            ...
            Vector<ServiceDescriptor> localServices = myContainer.getServiceManager().getLocalServices();
            String localNodeName = getLocalNode().getName();
            for (Slice value : slices) {
                try {
                    MainReplicationSlice slice = (MainReplicationSlice) value;
            ...
            Collections.addAll(replicas, temp);
    ...
    private class CommandOutgoingFilter extends Filter {
        ...
        public boolean accept(VerticalCommand cmd) {
            ...
            try {
                ...
                switch (name) {
                    case jade.core.management.AgentManagementSlice.ADD_TOOL -> handleNewTool(cmd);
                    case jade.core.management.AgentManagementSlice.REMOVE_TOOL -> handleDeadTool(cmd);

//#PJAVA_EXCLUDE_BEGIN
                    case jade.core.nodeMonitoring.UDPNodeMonitoringService.ORPHAN_NODE -> handleOrphanNode(cmd);
                }
                //#PJAVA_EXCLUDE_END
        ...
        public void postProcess(VerticalCommand cmd) {
            try {
                ...
                switch (name) {
                    ...
                    case jade.core.messaging.MessagingSlice.NEW_MTP -> handleNewMTP(cmd);
                    case jade.core.messaging.MessagingSlice.DEAD_MTP -> handleDeadMTP(cmd);
                }
            } catch (Throwable t) {
    ...
    private class ServiceComponent implements Slice, NodeEventListener {
        ...
        public VerticalCommand serve(HorizontalCommand cmd) {
            try {
                ...
                switch (cmdName) {
                    case MainReplicationSlice.H_GETLABEL -> {
                        Integer i = getLabel();
                        cmd.setReturnValue(i);
                    }
                    case MainReplicationSlice.H_INVOKESERVICEMETHOD -> {
                    ...
                    case MainReplicationSlice.H_GETPLATFORMMANAGERADDRESS -> cmd.setReturnValue(getPlatformManagerAddress());
                    case MainReplicationSlice.H_ADDREPLICA -> {
                        ...
                        Vector<ServiceDescriptor> services = (Vector<ServiceDescriptor>) params[4];
                        addReplica(sliceName, smAddr, sliceIndex, dsc, services);
                    }
                    case MainReplicationSlice.H_REMOVEREPLICA -> {
                        String smAddr = (String) params[0];
                        int sliceIndex = (Integer) params[1];
                        removeReplica(smAddr, sliceIndex);
                    }
                    case MainReplicationSlice.H_FILLGADT -> {
                        AID[] agents = (AID[]) params[0];
                        ContainerID[] containers = (ContainerID[]) params[1];
                        fillGADT(agents, containers);
                    }
                    case MainReplicationSlice.H_BORNAGENT -> {
                        AID name = (AID) params[0];
                        ContainerID cid = (ContainerID) params[1];
                        bornAgent(name, cid, cmd.getPrincipal(), cmd.getCredentials());
                    }
                    case MainReplicationSlice.H_DEADAGENT -> {
                        AID name = (AID) params[0];
                        deadAgent(name);
                    }
                    case MainReplicationSlice.H_SUSPENDEDAGENT -> {
                        AID name = (AID) params[0];
                        suspendedAgent(name);
                    }
                    case MainReplicationSlice.H_RESUMEDAGENT -> {
                        AID name = (AID) params[0];
                        resumedAgent(name);
                    }
                    case MainReplicationSlice.H_NEWMTP -> {
                        MTPDescriptor mtp = (MTPDescriptor) params[0];
                        ContainerID cid = (ContainerID) params[1];
                        newMTP(mtp, cid);
                    }
                    case MainReplicationSlice.H_DEADMTP -> {
                        MTPDescriptor mtp = (MTPDescriptor) params[0];
                        ContainerID cid = (ContainerID) params[1];
                        deadMTP(mtp, cid);
                    }
                    case MainReplicationSlice.H_NEWTOOL -> {
                        AID tool = (AID) params[0];
                        newTool(tool);
                    }
                    case MainReplicationSlice.H_DEADTOOL -> {
                        AID tool = (AID) params[0];
                        deadTool(tool);
                    }
                }
        ...
        private Method getMethod(Service svc, String methodName) throws Exception {
            ...
            Method m = cachedServiceMethods.get(key);
            if (m == null) {
        ...
        private void addReplica(String sliceName, String smAddr, int sliceIndex, NodeDescriptor dsc, Vector<ServiceDescriptor> services) throws IMTPException, ServiceException {
        ...
        private void fillGADT(AID[] agents, ContainerID[] containers) throws JADESecurityException {
            for (int i = 0; i < agents.length; i++) {
                ...
                } catch (NotFoundException | NameClashException nfe) {
                    // It should never happen...
                    nfe.printStackTrace();
                }
            }
        ...
        public void nodeRemoved(Node n) {
            ...
            switch (action) {
                case REMOVE_NODE -> removeTerminatedNode(n);
                case WAIT -> {
        ...
        private void removeTerminatedNode(Node n) {
            ...
            try {
                ...
                MainReplicationSlice newSlice = replicas.get(monitoredLabel);
