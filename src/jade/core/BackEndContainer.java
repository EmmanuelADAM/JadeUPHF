...
public class BackEndContainer extends AgentContainerImpl implements BackEnd {
    ...
    private static Properties adjustProperties(Properties pp) {
        ...
        if (feServices != null) {
            Vector<Object> ss = Specifier.parseList(pp.getProperty(Profile.SERVICES), ';');
            Vector<Object> fess = Specifier.parseList(feServices, ';');
            for (Object o : fess) {
                String s = (String) o;
                if (!ss.contains(s)) {
                    ss.add(s);
    ...
    public boolean connect() {
        try {
            ...
            Vector<Object> agentSpecs = Specifier.parseSpecifierList(myProfile.getParameter(Profile.AGENTS, null));
        ...
        try {
            ...
            for (Method method : mm) {
                if (method.getName().equals(methodName)) {
                    if (isCompatible(method, decodedParams)) {
                        m = method;
                        break;
                    }
                }
    ...
    public AID[] agentNames() {
        ...
        System.arraycopy(realAgents, 0, all, 0, realAgents.length);
        System.arraycopy(images, 0, all, realAgents.length, images.length);
        return all;
    }
    ...
    private void notifySynchronized() {
        synchronized (frontEndSynchLock) {
            Iterator<MessageSenderPair> it = fronEndSynchBuffer.iterator();
            while (it.hasNext()) {
                try {
                    MessageSenderPair msp = it.next();
