...
class Scheduler implements Serializable {
    ...
    public synchronized void restartAll() {
		...
		/*#MIDP_INCLUDE_BEGIN
			 ...
			 #MIDP_INCLUDE_END*/
        for (Behaviour readyBehaviour : readyBehaviours) behaviours[counter++] = readyBehaviour;

        for (Behaviour b : behaviours) b.restart();

        behaviours = new Behaviour[blockedBehaviours.size()];
        counter = 0;
        //#MIDP_EXCLUDE_BEGIN
        for (Behaviour blockedBehaviour : blockedBehaviours) {
            //#MIDP_EXCLUDE_END
			/*#MIDP_INCLUDE_BEGIN
            ...
            behaviours[counter++] = blockedBehaviour;
            //#DOTNET_EXCLUDE_END
