...
class FSPersistentStorage implements PersistentStorage {
    ...
    public void clear(final boolean clearPlatformInfo) throws Exception {
        ...
        File[] ff = locationDir.listFiles((dir, name) -> {
    ...
    public Map<String, Object> getAllNodes(boolean children) throws Exception {
        final String end = NODE_POSTFIX + (children ? CHILD_EXTENSION : EXTENSION);

        //#DOTNET_EXCLUDE_BEGIN
        File[] ff = locationDir.listFiles((dir, name) -> (name.endsWith(end)));
        //#DOTNET_EXCLUDE_END
        ...
        Map<String, Object> nodes = new HashMap<>(ff.length);
        for (File file : ff) {
            nodes.put(getNodeName(file.getName()), readContent(file));
        }
        return nodes;
    }
