...
public class AgentMobilityService extends BaseService {
    ...
    public Class<AgentMobilitySlice> getHorizontalInterface() {
    ...
    private class CommandSourceSink implements Sink {
        ...
        public void consume(VerticalCommand cmd) {
            try {
                ...
                switch (name) {
                    case AgentMobilityHelper.REQUEST_MOVE -> handleRequestMove(cmd);
                    case AgentMobilityHelper.REQUEST_CLONE -> handleRequestClone(cmd);
                    case AgentMobilityHelper.INFORM_MOVED -> handleInformMoved(cmd);
                    case AgentMobilityHelper.INFORM_CLONED -> handleInformCloned(cmd);
                }
            } catch (IMTPException | NotFoundException | NameClashException | JADESecurityException imtpe) {
        ...
        private void handleInformMoved(VerticalCommand cmd) throws IMTPException, ServiceException, JADESecurityException, NotFoundException {
            ...
            try {
                ...
                String classSiteName = sites.get(a);
    ...
    private class CommandTargetSink implements Sink {
        ...
        private void clonedAgent(AID agentID, ContainerID cid, Credentials credentials) throws JADESecurityException, NotFoundException, NameClashException {
            ...
            if (impl != null) {
                ...
                replicationHandle.invokeReplicatedMethod("bornAgent", new Object[]{agentID, cid, null, ownership, Boolean.TRUE});
            }
        }
    ...
    private class CommandOutgoingFilter extends Filter {
        ...
        protected boolean accept(VerticalCommand cmd) {
            ...
            if (name.equals(AgentManagementSlice.INFORM_KILLED)) {
                ...
                } catch (NotFoundException | ServiceException | IMTPException nfe) {
    ...
    private class ServiceComponent implements Slice {
        ...
        public VerticalCommand serve(HorizontalCommand cmd) {
            ...
            try {
                ...
                switch (cmdName) {
                    case AgentMobilitySlice.H_CREATEAGENT -> {
                        AID agentID = (AID) params[0];
                    ...
                    case AgentMobilitySlice.H_COPYAGENT -> {
                    ...
                    case AgentMobilitySlice.H_PREPARE -> cmd.setReturnValue(prepare());
                    case AgentMobilitySlice.H_TRANSFERIDENTITY -> {
                        AID agentID = (AID) params[0];
                        Location src = (Location) params[1];
                        Location dest = (Location) params[2];

                        cmd.setReturnValue(transferIdentity(agentID, src, dest));
                    }
                    case AgentMobilitySlice.H_HANDLETRANSFERRESULT -> {
                        AID agentID = (AID) params[0];
                        boolean transferResult = (Boolean) params[1];
                        List<ACLMessage> messages = (List<ACLMessage>) params[2];

                        handleTransferResult(agentID, transferResult, messages);
                    }
                    case AgentMobilitySlice.H_CLONEDAGENT -> {
            ...
            try {
                ...
                if (result == TRANSFER_ABORT) {
                    ...
                    for (int i = messages.size(); i > 0; i--) {
                        agent.putBack(messages.get(i - 1));
                    }
                ...
                if (ad != null) {
                    try {
                        ...
                        if (srcReady && destReady) {
                            ...
                            replicationHandle.invokeReplicatedMethod("movedAgent", new Object[]{agentID, src, dest});
                            return true;
                        } else {
    ...
    private class Deserializer extends ObjectInputStream {
        ...
        private final String classSiteName;
        ...
        protected Class<?> resolveClass(ObjectStreamClass v) throws ClassNotFoundException {
            ...
            MobileAgentClassLoader cl = loaders.get(key);
            if (cl == null) {
                try {
            ...
            try {
                ...
                c = primitiveJavaClasses.get(v.getName());
                if (c == null) {
                    throw ex;
                }
            }
