...
class PersistentDeliveryManager {
        ...
        try {
            ...
            storage.loadAll(new MessageStorage.LoadListener() {
                ...
                public void itemLoaded(String storeName, GenericMessage msg, AID receiver) {
                    ...
                    synchronized (pendingMessages) {
                        List<DeliveryItem> msgs = (List<DeliveryItem>) pendingMessages.get(receiver);
                        if (msgs == null) {
                            msgs = new LinkedList<>();
    ...
    public int flushMessages(AID receiver) {
        ...
        int cnt = 0;
        List<DeliveryItem> l = null;
        synchronized (pendingMessages) {
            l = (List<DeliveryItem>) pendingMessages.remove(receiver);
        }
        if (l != null) {
            for (DeliveryItem item : l) {
                retry(item);
                cnt++;
            }
        }
