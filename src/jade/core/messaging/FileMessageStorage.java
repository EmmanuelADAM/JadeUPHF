...
class FileMessageStorage implements MessageStorage {
    ...
    public synchronized void loadAll(LoadListener ll) throws IOException {
        ...
        File[] subdirs = baseDir.listFiles(f -> f.isDirectory() && f.getName().startsWith(RECEIVER_PREFIX));
        for (File subdir : subdirs) {

            // Scan all its valid files.
            File[] files = subdir.listFiles(f -> !f.isDirectory() && f.getName().startsWith(MESSAGE_PREFIX));
            ...
            for (File toRead : files) {
                ...
                try {
                    ...
                    ByteArrayInputStream istream = new ByteArrayInputStream(Base64.decodeBase64(encodedMsg.getBytes(StandardCharsets.US_ASCII)));
                ...
                } catch (ACLCodec.CodecException | ClassNotFoundException ce) {
    ...
    private void createMessageFile(File toStore, GenericMessage msg, AID receiver) throws IOException {
        ...
        try {
            ...
            p.writeObject(msg);
            String strMessage = new String(Base64.encodeBase64(ostream.toByteArray()), StandardCharsets.US_ASCII);
            ostream.close();
