...
public class IncomingEncodingFilter extends Filter {
    ...
    private final Map<String, ACLCodec> messageEncodings;
    private final MessagingService myService;

    public IncomingEncodingFilter(Map<String, ACLCodec> m, MessagingService svc) {
        messageEncodings = m;
        myService = svc;
...
//			}
    ...
    public ACLMessage decodeMessage(Envelope env, byte[] payload) throws MessagingService.UnknownACLEncodingException, ACLCodec.CodecException {
        ...
        if (enc != null) { // A Codec was selected
            ACLCodec codec = messageEncodings.get(enc.toLowerCase());
            if (codec != null) {
                // Supported Codec
                String charset;
