...
public class ProfileImpl extends Profile {
    ...
    public ProfileImpl(boolean isMain) {
		...
		/*#ALL_INCLUDE_BEGIN
		 ...
		 #ALL_INCLUDE_END*/
        props.setProperty(Profile.MAIN, String.valueOf(isMain)); // set to a main/non-main container
    }
     ...
     * @param value is the property value
     */
    public void setSpecifiers(String key, List<Specifier> value) {
        //#MIDP_EXCLUDE_BEGIN
        props.put(key, value);
        //#MIDP_EXCLUDE_END
    }
    ...
    public List<Specifier> getSpecifiers(String key) throws ProfileException {
        ...
        try {
            Vector<Object> v = Specifier.parseSpecifierList(specsLine);
            // convert the vector into an arraylist (notice that using the vector allows to avoid class loading of ArrayList)
            List<Specifier> l1 = new ArrayList<>(v.size());
            for (int i = 0; i < v.size(); i++)
                l1.add((Specifier) v.elementAt(i));
            return l1;
        } catch (Exception e) {
