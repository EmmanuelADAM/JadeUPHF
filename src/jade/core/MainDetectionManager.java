...
public class MainDetectionManager {
     ...
     */
    private static MainAddr manageGetMainResponses(List<DatagramPacket> responses, String proto) {
        ...
        String response;
        String s;
        Iterator<DatagramPacket> iter = responses.iterator();
        byte[] data;
        InetAddress senderHost;
        int senderPort;
        while (iter.hasNext()) {
            DatagramPacket p = iter.next();
            data = p.getData();
     ...
     */
    private static List<DatagramPacket> multicAsk(String request, Profile p) throws ProfileException {
        logger.log(Logger.FINER, "MainDetectionManager::multicAsk(...)");
        List<DatagramPacket> result = null;
        /*
         * usually we expect only one Main, but when more are active,
         * we collect all responses and choose the one we like more
         */
        List<DatagramPacket> responses = new ArrayList<>(3);
    ...
    private static MainAddr getMainAddress(Profile profile) throws ProfileException {
        ...
        List<DatagramPacket> responses = multicAsk(msg, profile);
        if (responses != null) {
            // parse the response
