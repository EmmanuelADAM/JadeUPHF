...
class MulticastMainDetectionListener implements Runnable {
    ...
    private final InetAddress mcastGroupAddress;
    ...
    private String serveGetMain(String request) throws IMTPException {
        ...
        if (responseAddresses.size() < 1) {
            response = new StringBuilder(errorResponse(MainDetectionManager.PROTO_RESP_NOTFOUND, "Cannot manage protocol " + proto));
        } else {
            response = new StringBuilder(MainDetectionManager.PROTO_RESP_OK);
            iter = responseAddresses.iterator();
            while (iter.hasNext()) {
                addr = iter.next();
                // FIXME use toString()
                //response += addr.toString();
                response.append(addr.getProto()).append(MainDetectionManager.PROTO_ADDR_SEPARATOR).append(addr.getHost()).append(MainDetectionManager.PROTO_ADDR_SEPARATOR).append(addr.getPort());
                if (iter.hasNext()) {
                    response.append(MainDetectionManager.PROTO_ADDRESSES_SEPARATOR);
                }
            }
        }
