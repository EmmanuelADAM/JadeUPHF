...
class SerializationEngine {
    ...
    private static Object deserializeObject(DataInputStream dis, byte[] data) throws LEAPSerializationException {
        try {
            ...
            return switch (id) {
                case NULL_ID -> null;
                case STRING_ID -> dis.readUTF();
                case ACL_ID -> LEAPACLCodec.deserializeACL(dis);
                case AID_ID -> LEAPACLCodec.deserializeAID(dis);
                case STRING_ARRAY_ID -> deserializeStringArray(dis);
                case BOOLEAN_ID -> dis.readBoolean();
                case INTEGER_ID -> dis.readInt();
                default ->
