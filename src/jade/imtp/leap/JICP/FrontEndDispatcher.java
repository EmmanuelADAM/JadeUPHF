    ...
    private class ConnectionReader extends Thread {
        ...
        private JICPPacket handleIncomingPacket(JICPPacket pkt) {
            switch (pkt.getType()) {
                ...
                case JICPProtocol.RESPONSE_TYPE, JICPProtocol.ERROR_TYPE -> {
                ...
                default -> myLogger.log(Logger.WARNING, myMediatorID + " - Unexpected incoming packet type: " + pkt.getType());
            }
            return null;
        }
    ...
    public void doTimeOut(final Timer t) {
        synchronized (timersLock) {
            if (t == kaTimer) {
                ...
                Thread thr = new Thread(() -> sendKeepAlive(t));
                thr.start();
            } else if (t == cdTimer) {
                // Send DROP_DOWN may take time --> Do it in a dedicated Thread
                Thread thr = new Thread(() -> dropDownConnection(t));
                thr.start();
            }
        }
    }
