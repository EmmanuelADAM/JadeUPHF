...
public class MicroStub {
    ...
    private Command executeRemotely(Command c, long timeout, int sessionId) throws IMTPException {
        ...
        try {
            ...
            if (r.getCode() == Command.ERROR) {
                if (!(Boolean) r.getParamAt(0)) {
                    // Unexpected exception thrown in the remote site
                    String msg = "Exception " + r.getParamAt(1) + " occurred in remote site processing command " + c.getCode() + ". " + r.getParamAt(2);
                    logger.log(Logger.SEVERE, msg);
                    throw new IMTPException(msg);
                } else if (r.getParamAt(1).equals("jade.core.IMTPException")) {
            ...
            if (timeout == 0) {
                ...
                if (timeout > 0) {
                    ...
                    timeout = (Math.max(remainingTime, MINIMUM_TIMEOUT));
                }
    ...
    private void postpone(Command c, long timeout, int sessionId, ICPException icpe) {
        if (logger.isLoggable(Logger.FINE)) {
            logger.log(Logger.FINE, Thread.currentThread() + ": Command " + c.getCode() + " postponed");
        }
        final PostponedCommand pc = new PostponedCommand(c, sessionId, icpe);
        pendingCommands.addElement(pc);
        if (timeout > 0) {
            logger.log(Logger.INFO, Thread.currentThread() + ": Activating Timer for Command " + c.getCode());
            pc.timer = TimerDispatcher.getTimerDispatcher().add(new Timer(System.currentTimeMillis() + timeout, t -> {
                ...
                if (t == pc.timer) {
                    logger.log(Logger.INFO, Thread.currentThread() + ": Timer for Command " + pc.command.getCode() + " expired!!!");
    ...
    private PostponedCommand removeFirst() {
        synchronized (pendingCommands) {
            PostponedCommand pc = null;
            if (pendingCommands.size() > 0) {
                pc = pendingCommands.elementAt(0);
