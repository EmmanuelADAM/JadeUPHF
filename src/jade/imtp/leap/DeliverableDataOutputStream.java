...
class DeliverableDataOutputStream extends DataOutputStream {
    ...
    private void serializeContainerIDArray(ContainerID[] cida) throws IOException, LEAPSerializationException {
        ...
        for (ContainerID containerID : cida) {
    ...
    private void serializeNode(Node n) throws LEAPSerializationException {
        try {
            ...
            serializeStub(stub);
        } catch (IOException | IMTPException ioe) {
            ioe.printStackTrace();
    ...
    private void serializeByteArray(byte[] ba)
        ...
        try {
            ...
            throw new LEAPSerializationException("IO error serializing byte[] " + Arrays.toString(ba));
        }
    }
    ...
    private void serializeEnvelope(Envelope e)
        ...
        try {
            ...
            // to
            Iterator<AID> it = e.getAllTo();
            while (it.hasNext()) {
                writeBoolean(true);
                serializeAID(it.next());
            }
            writeBoolean(false);
            ...
            // properties
            Iterator<Property> it2 = e.getAllProperties();
            //it = e.getAllProperties();
            while (it2.hasNext()) {
                writeBoolean(true);
                serializeProperty(it2.next());
            }
            writeBoolean(false);
        } catch (IOException ioe) {
    ...
    private Serializer getSerializer(Object o)
        ...
        String serName = "jade.imtp.leap." + name + "Serializer";
