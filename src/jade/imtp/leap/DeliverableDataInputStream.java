...
class DeliverableDataInputStream extends DataInputStream {
     ...
     *                                    deserialization.
     */
    public Vector<Object> readVector() throws LEAPSerializationException {
        try {
            boolean presenceFlag = readBoolean();
     ...
     *
     */
    public Vector<Object> deserializeVector() throws IOException, LEAPSerializationException {
        int size = readInt();
    ...
    private MultipleGenericMessage deserializeMultipleGenericMessage() throws IOException, LEAPSerializationException {
        ...
        int length = 0;
        List<GenericMessage> messages = new ArrayList<>(nMessages);
    ...
    private ArrayList<TransportAddress> deserializeArrayList() throws LEAPSerializationException {
        try {
            ArrayList<TransportAddress> l = new ArrayList<>();
            int size = readInt();
            for (int i = 0; i < size; ++i) {
                l.add((TransportAddress) readObject());
            }
            return l;
        } catch (IOException ioe) {
