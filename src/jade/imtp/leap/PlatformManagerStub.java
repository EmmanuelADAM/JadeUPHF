...
class PlatformManagerStub extends Stub implements PlatformManager {
    ...
    public String addNode(NodeDescriptor desc, Vector<ServiceDescriptor> services, boolean propagated) throws IMTPException, ServiceException, JADESecurityException {
        try {
            Command cmd = new Command(ADD_NODE, remoteID);

            cmd.addParam(desc);
            cmd.addParam(services);
            cmd.addParam(propagated);
            ...
            switch (checkResult(result, new String[]{AUTH_EXCEPTION, SERVICE_EXCEPTION})) {
                case 1 -> throw new JADESecurityException((String) result.getParamAt(1));
                case 2 -> throw new ServiceException((String) result.getParamAt(1));
            }
    ...
    public Vector<Service.Slice> findAllSlices(String serviceKey) throws IMTPException, ServiceException {
        try {
            ...
            return (Vector<Service.Slice>) res.getParamAt(0);
        } catch (DispatcherException de) {
            throw new IMTPException(DISP_ERROR_MSG, de);
        } catch (UnreachableException ue) {
            throw new IMTPException(UNRCH_ERROR_MSG, ue);
