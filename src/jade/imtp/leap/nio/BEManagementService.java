...
public class BEManagementService extends BaseService {
    ...
    private void handleAcceptOption(Profile p) throws ProfileException {
        ...
        if (acceptOption != null) {
            ...
            List<Specifier> acceptedProtocols = p.getSpecifiers(ACCEPT);
            // Do not overwrite explicitly specified servers if any
            StringBuilder serverIDs = new StringBuilder(p.getParameter(SERVERS, null));
            serverIDs = new StringBuilder((serverIDs != null ? serverIDs.append(';') : ""));
            for (int i = 0; i < acceptedProtocols.size(); ++i) {
                Specifier s = acceptedProtocols.get(i);
                ...
                if (proto.equalsIgnoreCase(MicroRuntime.SOCKET_PROTOCOL) ||
                    ...
                    serverIDs.append(manageAcceptedProtocol(s, proto.toLowerCase(), p)).append(semicolon);
                } else {
                    myLogger.log(Logger.WARNING, "Unsupported protocol " + proto + ". Permitted values are socket, ssl, http and https!!!!");
                }
            }
            p.setParameter(SERVERS, serverIDs.toString());
        }
    }
    ...
    public void boot(final Profile p) throws ServiceException {
        ...
        Vector<Object> v = Specifier.parseList(serverIDs, ';');
        ...
        Thread t = new Thread(() -> {
            ...
            Iterator<Map.Entry<String, IOEventServer>> it = servers.entrySet().iterator();
            while (it.hasNext()) {
                String id = null;
                try {
                    Map.Entry<String, IOEventServer> entry = it.next();
                    id = entry.getKey();
                    entry.getValue().activate();
                } catch (Throwable t1) {
                    myLogger.log(Logger.WARNING, "Error activating IOEventServer " + id + ". " + t1);
                    t1.printStackTrace();
                }
            }
        });
    ...
    private String encodeConfigOptionsResponse() throws Exception {
        ...
        pp.load(new FileReader(configOptionsFileName));
    ...
    private class IOEventServer implements PDPContextManager, PDPContextManager.Listener, JICPMediatorManager {
        ...
        public void init(String id, Profile p) {
            ...
            try {
                leapProps.load(new FileReader(fileName));
        ...
        public synchronized void activate() throws Throwable {
            ...
            for (LoopManager looper : loopers) {
                looper.start();
            }
        }
    ...
    private class LoopManager implements Runnable {
        ...
        private void handleAcceptOp(SelectionKey key, String prefix) {
            try {
        ...
        private void handleReadOp(SelectionKey key, String prefix) throws ICPException {
        ...
        private synchronized void register(SocketChannel sc) {
            pendingChannels.add(sc);
            pendingChannelPresent = true;
            mySelector.wakeup();
        }

        private synchronized void handlePendingChannels(String prefix) {
            if (pendingChannelPresent) {
                for (SocketChannel sc : pendingChannels) {
                    if (myLogger.isLoggable(Logger.FINE)) {
            ...
            while (active) {
                try {
                    ...
                    Thread t = new Thread(() -> {
                        Object[] ss = servers.values().toArray();
                        for (Object s : ss) {
                            ((IOEventServer) s).tick(currentTime);
                        }
                        if (myLogger.isLoggable(Logger.FINE)) {
    ...
    private class BEManagementHelperImpl implements BEManagementHelper {
        ...
        private NIOMediator findMediatorGlobally(String id) {
            Object[] ss = servers.values().toArray();
            for (Object s : ss) {
                NIOMediator m = ((IOEventServer) s).getFromID(id);
                if (m != null) {
                    return m;
                }
            }
