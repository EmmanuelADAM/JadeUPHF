...
public class RMIIMTPManager implements IMTPManager {
        ...
        try {
            ...
            if (myLogger.isLoggable(Logger.CONFIG)) {
                ...
                for (String name : names) {
                    sb.append("Name ").append(name).append(" bound to ").append(theRegistry.lookup(name)).append("\n");
                }
    ...
    public void unexportPlatformManager(PlatformManager mgr) throws IMTPException {
        if (myRMIServiceManager != null && myRMIServiceManager.getPlatformManager().equals(mgr)) {
            ...
            try {
                ...
                UnicastRemoteObject.unexportObject(myRMIServiceManager, true);
            } catch (Exception e) {
    ...
    public boolean compareAddresses(String addr1, String addr2) {
        return addr1.equalsIgnoreCase(addr2);
    }

    /**
     * Creates the client socket factory, which will be used
     * to instantiate a  UnicastRemoteObject  .
     *
        ...
        if (fieldEnd > 0) {
            ...
            if (fieldEnd > 0) {
                ...
                if (fieldEnd > 0) {
                    ...
                    // Anchor
                    anchor = url.substring(fieldStart);
                } else {

                    // No anchor is specified after the file
                    file = url.substring(fieldStart);
                }
            } else {

                // No file is specified after the port
                port = url.substring(fieldStart);
            }
        } else {
    ...
    private class PlatformManagerAdapter implements PlatformManager {
        ...
        private final ServiceManagerRMI adaptee;
