...
public class DFDBKB extends DBKB {
    ...
    private void saveAID(AID aid) throws SQLException {
        ...
        if (props.size() > 0) {
            ...
            for (Map.Entry<Object, Object> objectObjectEntry : props.entrySet()) {
                ...
                pss.stm_insAgentUserDefSlot.setString(3, (String) objectObjectEntry.getKey());
                pss.stm_insAgentUserDefSlot.setString(4, (String) objectObjectEntry.getValue());
        ...
        // Resolvers
        Iterator<AID> iter2 = aid.getAllResolvers();

        if (iter2.hasNext()) {
            pss.stm_insAgentResolverAID.clearBatch();
            while (iter2.hasNext()) {
                AID resolverAID = iter2.next();
    ...
    private void saveServices(String descrId, Iterator<ServiceDescription> iter) throws SQLException {
        if (iter.hasNext()) {
            ...
            while (iter.hasNext()) {
                ...
                // Service - Protocols
                Iterator<String> iterS = service.getAllProtocols();
                while (iterS.hasNext()) {
                    pss.stm_insServiceProtocol.setString(1, serviceId);
                    pss.stm_insServiceProtocol.setString(2, iterS.next());
                ...
                while (iterS.hasNext()) {
                    ...
                    pss.stm_insServiceOntology.setString(2, iterS.next());
                ...
                while (iterServ.hasNext()) {
                    ...
                    if (propObj instanceof MultiValueProperty mvProp) {
                        ...
                        try {
                            saveProperty(pss, serviceId, propObj);
                            executePropertiesBatch = true;
                        } catch (Exception e) {
                            if (logger.isLoggable(Logger.SEVERE))
                                logger.log(Logger.SEVERE, "Cannot serialize property '" + propObj.getName() +
                                        "' for service '" + service.getName() + "'", e);
                        }
                    }
                }
    ...
    protected Object insertSingle(Object name, Object fact) throws SQLException {
        ...
        try {
            ...
            // Languages
            Iterator<String> iter = dfd.getAllLanguages();
            if (iter.hasNext()) {
                pss.stm_insLanguage.clearBatch();
                while (iter.hasNext()) {
                    pss.stm_insLanguage.setString(1, descrId);
                    pss.stm_insLanguage.setString(2, iter.next());
            ...
            if (iter.hasNext()) {
                ...
                while (iter.hasNext()) {
                    ...
                    pss.stm_insOntology.setString(2, iter.next());
            ...
            if (iter.hasNext()) {
                ...
                while (iter.hasNext()) {
                    ...
                    pss.stm_insProtocol.setString(2, iter.next());
    ...
    protected List<DFAgentDescription> searchSingle(DFAgentDescription template, int maxResult) throws SQLException {
        ...
        List<DFAgentDescription> dfds = new ArrayList<>(matchingAIDs.size());
        Iterator<String> it = matchingAIDs.iterator();
        // FIXME: Define a proper constant and possibly a proper configuration option
        if (matchingAIDs.size() < 10) {
            while (it.hasNext()) {
                dfds.add(getDFD(it.next()));
            }
        } else {
            ...
            PreparedStatements pss = getPreparedStatements();
            Map<String, List<String>> allLanguages = preloadIdValueTable(pss.stm_selCountAllLanguages, pss.stm_selAllLanguages);
            Map<String, List<String>> allOntologies = preloadIdValueTable(pss.stm_selCountAllOntologies, pss.stm_selAllOntologies);
            Map<String, List<String>> allProtocols = preloadIdValueTable(pss.stm_selCountAllProtocols, pss.stm_selAllProtocols);
            while (it.hasNext()) {
                dfds.add(getDFD(it.next(), allLanguages, allOntologies, allProtocols));
            }
        }
    ...
    private Map<String, List<String>> preloadIdValueTable(PreparedStatement cntStm, PreparedStatement stm) throws SQLException {
    ...
    private AID getAID(String aidN) throws SQLException {
        ...
        // AID resolvers
        Collection<String> resolvers = getResolverAIDs(aidN);
        for (String resolver : resolvers) {
            id.addResolvers(getAID(resolver));
        }
     ...
     */
    private DFAgentDescription getDFD(String aidN, Map<String, List<String>> allLanguages, Map<String, List<String>> allOntologies, Map<String, List<String>> allProtocols) throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (!multiValues.isEmpty()) {
                    for (Object o : multiValues.values()) {
                        sd.addProperties((MultiValueProperty) o);
                    }
                }
    ...
    private void loadLanguages(String descrId, DFAgentDescription dfd, Map<String, List<String>> allLanguages) throws SQLException {
        if (allLanguages != null) {
            List<String> langs = allLanguages.get(descrId);
            if (langs != null) {
                for (String lang : langs) {
                    dfd.addLanguages(lang);
                }
            }
        } else {
            PreparedStatements pss = getPreparedStatements();
    ...
    private void loadProtocols(String descrId, DFAgentDescription dfd, Map<String, List<String>> allProtocols) throws SQLException {
        if (allProtocols != null) {
            List<String> protos = allProtocols.get(descrId);
            if (protos != null) {
                for (String proto : protos) {
                    dfd.addProtocols(proto);
                }
            }
        } else {
            PreparedStatements pss = getPreparedStatements();
    ...
    private void removeAID(String aid) throws SQLException {
        ...
        if (found == 0) {
            ...
            // resolvers
            Collection<String> resolverAIDs = getResolverAIDs(aid);
            for (String resolverAID : resolverAIDs) {
                removeAID(resolverAID);
            }
    ...
    private String createSelect(DFAgentDescription dfdTemplate) throws Exception {
        ...
        StringBuilder select = new StringBuilder("SELECT dfagentdescr.aid FROM dfagentdescr");

        List<String> lAs = new ArrayList<>();
        List<String> lWhere = new ArrayList<>();
        ...
        // Languages
        Iterator<String> iter = dfdTemplate.getAllLanguages();
        int i = 0;
        while (iter.hasNext()) {
            String tmp = LANGUAGE + i;
            lAs.add(", language " + tmp);
            lWhere.add(tmp + ".language='" + iter.next() + "'");
        ...
        while (iter.hasNext()) {
            ...
            lWhere.add(tmp + ".ontology='" + iter.next() + "'");
            lWhere.add(tmp + ".descrid=dfagentdescr.id");
            i++;
        }
        // Protocols
        ...
        while (iter.hasNext()) {
            ...
            lWhere.add(tmp + ".protocol='" + iter.next() + "'");
            lWhere.add(tmp + ".descrid=dfagentdescr.id");
            i++;
        }
        // Services
        ...
        while (iter.hasNext()) {
            ...
            // Service languages
            Iterator<String> iterS = service.getAllLanguages();
            int j = 0;
            while (iterS.hasNext()) {
                String tmp1 = SERVICELANGUAGE + j;
                lAs.add(", servicelanguage " + tmp1);
                lWhere.add(tmp1 + ".language='" + iterS.next() + "'");
            ...
            while (iterS.hasNext()) {
                ...
                lWhere.add(tmp1 + ".ontology='" + iterS.next() + "'");
                lWhere.add(tmp1 + ".serviceid=" + tmp + ".id");
                j++;
            }
            // Service protocols
            ...
            while (iterS.hasNext()) {
                ...
                lWhere.add(tmp1 + ".protocol='" + iterS.next() + "'");
                lWhere.add(tmp1 + ".serviceid=" + tmp + ".id");
                j++;
            }
            // Service properties
            Iterator<Property> iterS2 = service.getAllProperties();
            j = 0;
            while (iterS.hasNext()) {
                String tmp1 = SERVICEPROPERTY + j;
                lAs.add(", serviceproperty " + tmp1);
                Property prop = iterS2.next();
    ...
    private void registerSubscription(String convID, String aclM) throws SQLException {
        ...
        try {
            PreparedStatements pss = getPreparedStatements();
            String base64Str = new String(Base64.encodeBase64(aclM.getBytes(StandardCharsets.US_ASCII)), StandardCharsets.US_ASCII);
    ...
    public Enumeration<SubscriptionResponder.Subscription> getSubscriptions() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String aclmStr = new String(Base64.decodeBase64(base64Str.getBytes(StandardCharsets.US_ASCII)), StandardCharsets.US_ASCII);
    ...
    private Object deserializeObj(String str) throws IOException, ClassNotFoundException {
        if (str == null)
            return null;
        byte[] data = Base64.decodeBase64(str.getBytes(StandardCharsets.US_ASCII));
    ...
    protected String getHashValue(Object obj) throws Exception {
        ...
        try {
            ...
            return new String(Base64.encodeBase64(digest), StandardCharsets.US_ASCII);
