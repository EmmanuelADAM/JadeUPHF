...
public class df extends GuiAgent implements DFGUIAdapter {
    ...
    protected void setup() {
        ...
        if (args != null && args.length > 0) {
            ...
            try {
                Reader targetReader = new StringReader((String) args[0]);
                p.load(targetReader);
        ...
        while (ss.hasMoreElements()) {
            Subscription s = (Subscription) ss.nextElement();
    ...
    protected void unsubscribeAgent(AID id) {
        List<Subscription> ss = dfSubscriptionResponder.getSubscriptions(id);
        for (Subscription s : ss) {
            try {
                subManager.deregister(s);
            } catch (Exception e) {
     ...
     */
    private void performRecursiveSearch(List<DFAgentDescription> localResults, DFAgentDescription dfd, SearchConstraints constraints, Search action) {
        ...
        newConstr.setMaxDepth((long) (maxDep - 1));
        // Max-results decreased by the number of items found locally
        newConstr.setMaxResults((long) (maxRes - localResults.size()));
    ...
    protected List<DFAgentDescription> DFSearch(DFAgentDescription dfd, int maxResults) {
     ...
     */
    protected List<DFAgentDescription> searchAction(Search s, AID requester) throws FIPAException {
     ...
     *
     * @throws FailureException If the GUI is already visible or some
     ...
     */
    List<AID> getParentsAction(GetParents action, AID requester) {
        if (logger.isLoggable(Logger.CONFIG))
     ...
     * Package scoped since it is called by DFAppletManagementBehaviour.
     */
    List<DFAgentDescription> getDescriptionAction(GetDescription action, AID requester) {
        if (logger.isLoggable(Logger.CONFIG))
        ...
        List<DFAgentDescription> tmp = new ArrayList<>();
     ...
     * Package scoped since it is called by DFAppletManagementBehaviour.
     */
    List<DFAgentDescription> getDescriptionUsedAction(GetDescriptionUsed action, AID requester) {
                    ...
                    Behaviour b = new RemoteDFRequester(df, s) {
                        public int onEnd() {
                            ...
                            if (!(result instanceof InternalError)) {
                                ...
                                gui.refreshLastSearchResults((ArrayList<DFAgentDescription>) result, getRemoteDF());
                            } else {
                                gui.showStatusMsg("Error processing request. " + ((InternalError) result).getMessage());
                            }
                            return 0;
    ...
    public DFAgentDescription getDFAgentDsc(AID name) throws FIPAException {
        ...
        List<DFAgentDescription> l = agentDescriptions.search(template, 1);
        if (l.isEmpty())
            return null;
        else
            return l.get(0);
    ...
    public DFAgentDescription getDescriptionOfThisDF(AID parent) {
        return dscDFParentMap.get(parent);
    }
    ...
    protected boolean showGui() {
        if (gui == null) {
            try {
                Class<DFGUIInterface> c = (Class<DFGUIInterface>) Class.forName("jade.tools.dfgui.DFGUI");
                gui = c.getDeclaredConstructor().newInstance();
                ...
                List<DFAgentDescription> agents = agentDescriptions.search(matchEverything, -1);
                List<AID> AIDList = new ArrayList<>();
                for (DFAgentDescription agent : agents) AIDList.add(agent.getName());
    ...
    private void sendPendingNotification(Concept action, Object result) {
        ACLMessage request = pendingRequests.remove(action);
        if (request != null) {
            ...
            try {
                ...
                send(notification);
                AID receiver = notification.getAllReceiver().next();
     ...
     *
     * @throws FIPAException if the search id is already in the cache.
     */
