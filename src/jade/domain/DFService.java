...
//            ServiceDescription sd;
    ...
    private static DFAgentDescription parseDfd(SimpleSLTokenizer parser) throws Exception {
        ...
        while (parser.nextToken().startsWith(":")) {
            ...
            // Name
            switch (slotName) {
                case FIPAManagementVocabulary.DFAGENTDESCRIPTION_NAME -> {
                    parser.consumeChar('(');
                    dfd.setName(parseAID(parser));
                }
                // Lease time
                case FIPAManagementVocabulary.DFAGENTDESCRIPTION_LEASE_TIME -> dfd.setLeaseTime(ISO8601.toDate(parser.getElement()));

                // Protocols
                case FIPAManagementVocabulary.DFAGENTDESCRIPTION_PROTOCOLS -> {
                    for (Object o : parseAggregate(parser)) {
                        dfd.addProtocols((String) o);
                    }
                }

                // Languages
                case FIPAManagementVocabulary.DFAGENTDESCRIPTION_LANGUAGES -> {
                    for (Object o : parseAggregate(parser)) {
                        dfd.addLanguages((String) o);
                    }
                }

                // Ontologies
                case FIPAManagementVocabulary.DFAGENTDESCRIPTION_ONTOLOGIES -> {
                    for (Object o : parseAggregate(parser)) {
                        dfd.addOntologies((String) o);
                    }
                }

                // Services
                case FIPAManagementVocabulary.DFAGENTDESCRIPTION_SERVICES -> {
                    for (Object o : parseAggregate(parser)) {
                        dfd.addServices((ServiceDescription) o);
                    }
                }
            }
        }
    ...
    private static ServiceDescription parseServiceDescription(SimpleSLTokenizer parser) throws Exception {
        ...
        while (parser.nextToken().startsWith(":")) {
            ...
            // Name
            switch (slotName) {
                case FIPAManagementVocabulary.SERVICEDESCRIPTION_NAME -> sd.setName(parser.getElement());

                // Type
                case FIPAManagementVocabulary.SERVICEDESCRIPTION_TYPE -> sd.setType(parser.getElement());

                // Ownership
                case FIPAManagementVocabulary.SERVICEDESCRIPTION_OWNERSHIP -> sd.setOwnership(parser.getElement());

                // Protocols
                case FIPAManagementVocabulary.SERVICEDESCRIPTION_PROTOCOLS -> {
                    for (Object o : parseAggregate(parser)) {
                        sd.addProtocols((String) o);
                    }
                }

                // Languages
                case FIPAManagementVocabulary.SERVICEDESCRIPTION_LANGUAGES -> {
                    for (Object o : parseAggregate(parser)) {
                        sd.addLanguages((String) o);
                    }
                }

                // Ontologies
                case FIPAManagementVocabulary.SERVICEDESCRIPTION_ONTOLOGIES -> {
                    for (Object o : parseAggregate(parser)) {
                        sd.addOntologies((String) o);
                    }
                }

                // Properties
                case FIPAManagementVocabulary.SERVICEDESCRIPTION_PROPERTIES -> {
                    for (Object o : parseAggregate(parser)) {
                        sd.addProperties((Property) o);
                    }
                }
            }
        }
    ...
    public static AID parseAID(SimpleSLTokenizer parser) throws Exception {
        ...
        while (parser.nextToken().startsWith(":")) {
            ...
            // Name
            switch (slotName) {
                case SL0Vocabulary.AID_NAME -> id.setName(parser.getElement());

                // Addresses
                case SL0Vocabulary.AID_ADDRESSES -> {
                    for (Object o : parseAggregate(parser)) {
                        id.addAddresses((String) o);
                    }
                }

                // Resolvers
                case SL0Vocabulary.AID_RESOLVERS -> {
                    for (Object o : parseAggregate(parser)) {
                        id.addResolvers((AID) o);
                    }
                }
            }
        }
    ...
    private static List<Object> parseAggregate(SimpleSLTokenizer parser) throws Exception {
        ...
        while (!next.startsWith(")")) {
            if (!next.startsWith("(")) {
                ...
                switch (next) {
                    case FIPAManagementVocabulary.DFAGENTDESCRIPTION -> l.add(parseDfd(parser));
                    case SL0Vocabulary.AID -> l.add(parseAID(parser));
                    case FIPAManagementVocabulary.SERVICEDESCRIPTION -> l.add(parseServiceDescription(parser));
                    case FIPAManagementVocabulary.MULTI_VALUE_PROPERTY -> l.add(parseMultiValueProperty(parser));
                    case FIPAManagementVocabulary.PROPERTY -> l.add(parseProperty(parser));
                }
            }
            next = parser.nextToken();
        }
     ...
     * @throws FIPAException A suitable exception can be thrown
