...
public class RequestFIPAServiceBehaviour extends SimpleAchieveREInitiator {
     ...
     * @param actionName       The action requested to the DF.
     * @param agentDescription An agent descriptor that will be use according to the action required to the DF.
     ...
     */
    protected void handleAllResponses(Vector<ACLMessage> reply) {
        notYetReady = false;
        if (reply.size() == 0)
            //the timeout has expired: no replies received.
            lastMsg = new ACLMessage(ACLMessage.NOT_UNDERSTOOD);
        else
            lastMsg = (ACLMessage) (reply.elementAt(0)).clone();
    }
     ...
     *                       performative.
     * @throws NotYetReady   is thrown if the protocol is not yet finished.
     **/
     ...
     */
    public List<Object> getSearchResult() throws FIPAException, NotYetReady {
    ...
    public Object[] getSearchResults() throws FIPAException, NotYetReady {
        ...
        List<Object> l;
        try {
            Result r = (Result) myAgent.getContentManager().extractContent(lastMsg);
            l = r.getItems();
        } catch (Exception e) {
            e.printStackTrace();
            throw new UnrecognisedValue("content");
