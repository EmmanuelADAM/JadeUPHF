...
public class ams extends Agent /*implements AgentManager.Listener*/ {
    ...
    private final Hashtable<Object, ACLMessage> pendingNewAgents = new Hashtable<>();
    private final Hashtable<Object, ACLMessage> pendingDeadAgents = new Hashtable<>();
    private final Hashtable<Object, ACLMessage> pendingClonedAgents = new Hashtable<>();
    private final Hashtable<Object, ACLMessage> pendingMovedAgents = new Hashtable<>();
    private final Hashtable<Object, ACLMessage> pendingRemovedContainers = new Hashtable<>();
    ...
    // CREATE AGENT
    void createAgentAction(final CreateAgent ca, final AID requester, final JADEPrincipal requesterPrincipal, final Credentials requesterCredentials) {
        ...
        ArrayList<Object> listArg = new ArrayList<>();
        while (it.hasNext()) {
            listArg.add(it.next());
        }
    ...
    // KILL CONTAINER
    void killContainerAction(final KillContainer kc, final AID requester, final JADEPrincipal requesterPrincipal, final Credentials requesterCredentials) {
    ...
    // INSTALL MTP
    MTPDescriptor installMTPAction(InstallMTP im, AID requester) throws FIPAException, MTPException {
        if (logger.isLoggable(Logger.FINE))
    ...
    // UNINSTALL MTP
    void uninstallMTPAction(UninstallMTP um, AID requester) throws FIPAException, MTPException {
        if (logger.isLoggable(Logger.FINE))
    ...
    List<ContainerID> queryPlatformLocationsAction(QueryPlatformLocationsAction qpl, AID requester) throws FIPAException {
        if (logger.isLoggable(Logger.FINE))
    ...
    List<AID> queryAgentsOnLocationAction(QueryAgentsOnLocation qaol, AID requester) throws FIPAException {
        if (logger.isLoggable(Logger.FINE))
    ...
    // SEARCH
    List<APDescription> searchAction(Search s, AID requester) {
        if (logger.isLoggable(Logger.FINE))
            logger.log(Logger.FINE, "Agent " + requester + " requesting AMS-search");
        return (List<APDescription>) myPlatform.amsSearch((AMSAgentDescription) s.getDescription(), getActualMaxResults(s.getConstraints()));
    }
    ...
    private long getActualMaxResults(SearchConstraints constraints) {
        long maxResult = (constraints.getMaxResults() == null ? 1 : constraints.getMaxResults());
    ...
    APDescription getDescriptionAction(AID requester) {
        ...
        MTPDescriptor dr;
        for (MTPDescriptor mtpDescriptor : platformMTPs()) {
            dr = mtpDescriptor;
            ...
            if (current != null) {
                ...
                try {
                    ...
                    for (ContainerID cid : ids) {
                        ...
                        for (MTPDescriptor mtpDescriptor : myPlatform.containerMTPs(cid)) {
                            AddedMTP amtp = new AddedMTP();
                            amtp.setAddress(mtpDescriptor.getAddresses()[0]);
                            amtp.setWhere(cid);
    ...
    private class EventManager extends CyclicBehaviour {
        ...
        public EventManager() {
            handlers.put(RemovedContainer.NAME, ev -> {
            ...
            handlers.put(DeadAgent.NAME, ev -> {
                // If this event was forced by an action requested by
                // an agent --> notify him.
            ...
            handlers.put(MovedAgent.NAME, ev -> {
                // If this event was forced by an action requested by
                // an agent --> notify him.
            ...
            handlers.put(BornAgent.NAME, ev -> {
                // If this event was forced by an action requested by
                // an agent --> notify him.
            ...
            handlers.put(PlatformDescription.NAME, ev -> {
    ...
    private List<MTPDescriptor> platformMTPs() {
        ...
        for (ContainerID containerID : cc) {
            try {
                List<MTPDescriptor> l = myPlatform.containerMTPs(containerID);
                mtps.addAll(l);
            } catch (NotFoundException nfe) {
