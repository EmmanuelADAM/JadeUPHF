...
public class Boot3 {
        ...
        if ((args == null) || (args.length == 0)) {
            ...
            for (String arg : args) {
                if (arg.startsWith("import:")) {
                    int j = arg.indexOf(':');
                    isNew = ((j < arg.length() - 1) && (isFileName(arg.substring(j + 1))));
                    likely = !isNew;  // in case malformed file name
                } else if (arg.startsWith("agents:")) {
                    isNew = true;
                } else if (arg.startsWith("-") && likely) {
                    isNew = true;
                }
            }
    ...
    public void listProperties(PrintStream out) {
        ...
        for (Enumeration<Object> e = properties.sortedKeys(); e.hasMoreElements(); ) {
