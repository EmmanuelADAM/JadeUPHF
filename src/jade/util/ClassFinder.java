...
public class ClassFinder {
     ...
     */
    public final Vector<Class<?>> findSubclasses(String fqcn, ClassFinderListener aListener, ClassFinderFilter aFilter) {
        synchronized (locations) {
    ...
    private void includeJar(File file, Map<URL, String> map) {
        ...
        while (e.hasMoreElements()) {
            ...
            if (entry.isDirectory()) {
                if (entry.getName().equalsIgnoreCase("META-INF/"))
                    continue;
    ...
    private Vector<Class<?>> findSubclasses(Class<?> superClass, Map<URL, String> locations) {
        Set<Class<?>> setOfClasses = new TreeSet<>(CLASS_COMPARATOR);

        for (URL url : locations.keySet()) {
            findSubclasses(url, locations.get(url), superClass, setOfClasses);
        }
        return new Vector<>(setOfClasses);
    }

    private void manageClass(Set<Class<?>> setOfClasses, Class<?> superClass, Class<?> c, URL url) {
        boolean include;
    ...
    private void findSubclasses(URL location, String packageName, Class<?> superClass, Set<Class<?>> setOfClasses) {
